...
class TestTableCreation:
    ...
    def test_tables_created_on_init(self, db):
        """StockDB __init__ 时应自动创建所有 7 张长表"""
        tables = get_table_names(db)
        expected = [
            "kline_daily", "factor_indicator", "factor_trend",
            "factor_momentum", "factor_volume", "factor_risk", "factor_ma_ratio",
        ]
        for t in expected:
            assert t in tables, f"表 {t} 未被创建"
...
class TestDropAllTables:
    ...
    def test_drop_all_tables(self, db):
        ...
        assert len(get_table_names(db)) == 7
        db.drop_all_tables()
    ...
    def test_drop_removes_indexes(self, db):
        """删表应同时删除索引"""
        assert len(get_index_names(db)) == 7
        db.drop_all_tables()
        assert get_index_names(db) == []
