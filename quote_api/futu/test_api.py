# -*- coding: utf-8 -*-
"""富途 OpenAPI QuoteAPI 手工冒烟测试。

运行前需启动并登录 OpenD：
    python quote_api/futu/test_api.py Tencent
"""

from __future__ import annotations

import os
import sys
from typing import Optional

_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
_PROJECT_ROOT = os.path.abspath(os.path.join(_THIS_DIR, "..", ".."))
if _PROJECT_ROOT not in sys.path:
    sys.path.insert(0, _PROJECT_ROOT)

from quote_api.futu import FutuQuoteAPI  # noqa: E402


SOURCE_NAME = "futu"
DEFAULT_STOCK = "Tencent"


def run_self_test(name: Optional[str] = None) -> bool:
    name = name or DEFAULT_STOCK
    print("=" * 60)
    print("[%s] self-test for stock=%s" % (SOURCE_NAME, name))
    print("=" * 60)

    with FutuQuoteAPI() as api:
        quote = api.get_daily_quote(name)
        print("[daily_quote] ->", quote)
        ok_daily = quote is not None and quote.is_valid()

        klines = api.get_klines(name, limit=5)
        print("[klines] count=%d" % len(klines))
        for item in klines:
            print("   ", item)
        ok_kline = len(klines) == 5

    passed = ok_daily and ok_kline
    print("-" * 60)
    print("[%s] result: %s" % (SOURCE_NAME, "PASS" if passed else "FAIL"))
    return passed


if __name__ == "__main__":
    arg = sys.argv[1] if len(sys.argv) > 1 else None
    sys.exit(0 if run_self_test(arg) else 1)
